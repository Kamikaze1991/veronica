...
class envio implements IModelo
{


	//definicion de datos
	const TABLE_NAME="envio";
	const ALL_FIELDS="ID,IDCLIENTE,IDRESPONSABLE,IDCATEGORIA,FECHA,CIUDADORIGEN,CIUDADDESTINO,SUBTOTAL,IVA,TOTAL,ESTADO";
	...
	private function crear($_array_data)
	{
		...
		try{
			...
			if($_array_data)
			{
				...
				$_rs=conexion::getInstancia()->GenericQueryProcedure("call PROCESAR_ENVIO(?,?,?,?,?,?,?)",$_array_data);	
