...
class cliente implements IModelo
{


	//definicion de datos
	const TABLE_NAME="cliente";
	const ALL_FIELDS="ID,CEDULA,NOMBRE,APELLIDO,DIRECCION";
	...
	private function crear($_array_data)
	{
		...
		try{
			...
			if($_array_data)
			{
				...
				$_rs=conexion::getInstancia()->GenericQueryProcedure("call INGRESO_CLIENTE(?,?,?,?)",$_array_data);	
